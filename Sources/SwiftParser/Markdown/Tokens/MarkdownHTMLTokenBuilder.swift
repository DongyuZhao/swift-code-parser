    ...
    public func build(from context: inout CodeTokenContext<MarkdownTokenElement>) -> Bool {
        ...
        if first == "<" {
            ...
            if context.source[start...].hasPrefix("<!--") {
                if let endRange = context.source.range(of: "-->", range: context.source.index(start, offsetBy: 4)..<context.source.endIndex) {
                    let end = endRange.upperBound
                    context.tokens.append(MarkdownToken.htmlComment(String(context.source[start..<end]), at: start..<end))
            ...
            let nameStart = idx
            while idx < context.source.endIndex && (context.source[idx].isLetter || context.source[idx].isNumber || context.source[idx] == "-") {
                idx = context.source.index(after: idx)
            }
            ...
            if isClosing || isSelfClosing {
                context.tokens.append(MarkdownToken.htmlTag(String(context.source[openTagRange]), at: openTagRange))
            ...
            if let closeRange = remaining.range(of: closingPattern, options: [.caseInsensitive]) {
                ...
                context.tokens.append(MarkdownToken.htmlBlock(String(context.source[start..<blockEnd]), at: start..<blockEnd))
                context.consuming = blockEnd
                return true
            }

            context.tokens.append(MarkdownToken.htmlUnclosedBlock(String(context.source[openTagRange]), at: openTagRange))
            context.consuming = endIdx
            return true
        } else if first == "&" {
            var idx = context.source.index(after: start)
            if idx < context.source.endIndex && context.source[idx] == "#" {
                idx = context.source.index(after: idx)
                if idx < context.source.endIndex && (context.source[idx] == "x" || context.source[idx] == "X") {
                    idx = context.source.index(after: idx)
                ...
                let nameStart = idx
                while idx < context.source.endIndex && (context.source[idx].isLetter || context.source[idx].isNumber) {
                    idx = context.source.index(after: idx)
                }
                if idx < context.source.endIndex && context.source[idx] == ";" {
