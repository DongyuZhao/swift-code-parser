...
public class MarkdownDefinitionListBuilder: CodeNodeBuilder {
  ...
  public func build(
    ...
    guard idx < context.tokens.count,
      ...
      (context.tokens[idx] as! MarkdownToken).element == .newline
    else {
    ...
    var termContext = CodeConstructContext(
      current: termNode, tokens: termTokens, state: context.state)
    ...
    var defContext = CodeConstructContext(
      current: descNode, tokens: defTokens, state: context.state)
