...
struct MarkdownBlankLinesTests {
  ...
  func multipleConsecutiveBlankLinesHaveSameEffectAsSingleBlankLine() {
    ...
    let expectedSig =
      #"document[paragraph[text("first paragraph")],paragraph[text("second paragraph")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func blankLinesBetweenDifferentBlockElementTypes() {
    ...
    let expectedSig =
      #"document[paragraph[text("paragraph")],heading(level:2)[text("heading")],blockquote[paragraph[text("blockquote")]],unordered_list(level:1)[list_item[paragraph[text("list item")]]]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func blankLinesSeparateParagraphsButNotPreservedAsNodes() {
    ...
    let expectedSig =
      #"document[paragraph[text("first")],paragraph[text("second")],paragraph[text("third")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func blankLinesBeforeAndAfterThematicBreaks() {
    ...
    let expectedSig =
      #"document[paragraph[text("paragraph")],thematic_break,paragraph[text("another paragraph")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func mixedBlankLinesAndContentPreserveOnlyMeaningfulStructure() {
    ...
    let expectedSig =
      #"document[heading(level:1)[text("Header")],paragraph[text("Content paragraph.")],heading(level:2)[text("Subheader")],paragraph[text("More content.")]]"#
    #expect(sig(result.root) == expectedSig)
  }
