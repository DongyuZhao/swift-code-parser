...
struct MarkdownRawHTMLTests {
  ...
  func htmlTagsWithWhitespace() {
    ...
    // Verify AST structure using sig
    let expectedSig = #"document[paragraph[html("<a  />"),html("<b2\#ndata="foo" >")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func htmlTagsWithAttributes() {
    ...
    // Verify AST structure using sig
    let expectedSig =
      #"document[paragraph[html("<a foo="bar" bam = 'baz <em>"</em>'\#n_boolean zoop:33=zoop:33 />")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func customTagNames() {
    ...
    // Verify AST structure using sig
    let expectedSig =
      #"document[paragraph[text("Foo "),html("<responsive-image src="foo.jpg" />")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func htmlComments() {
    ...
    // Verify AST structure using sig
    let expectedSig =
      #"document[paragraph[text("foo "),html("<!-- this is a --\#ncomment - with hyphens -->")]]"#
    #expect(sig(result.root) == expectedSig)
  }
  ...
  func specialHTMLCommentForms() {
    ...
    // Verify AST structure using sig
    let expectedSig =
      #"document[paragraph[text("foo "),html("<!-->"),text(" foo -->")],paragraph[text("foo "),html("<!--->"),text(" foo -->")]]"#
    #expect(sig(result.root) == expectedSig)
  }
