...
@Suite("Markdown Tokenizer Complex Tests")
struct MarkdownTokenizerComplexTests {
  ...
  func testComplexMarkdownStructures() {
    let testCases: [(String, [MarkdownTokenElement], String)] = [
      (
        ...
        [
          .hash, .space, .text, .space, .text, .space, .asterisk, .asterisk, .text, .asterisk,
          .asterisk, .space, .text, .space, .asterisk, .text, .asterisk, .eof,
        ],
      ...
      (
        ...
        [
          .text, .space, .text, .space, .inlineCode, .space, .text, .space, .asterisk, .asterisk,
          .text, .asterisk, .asterisk, .space, .text, .eof,
        ],
      ...
      (
        ...
        [
          .text, .space, .text, .space, .leftBracket, .text, .rightBracket, .leftParen, .text,
          .rightParen, .space, .text, .space, .exclamation, .leftBracket, .text, .rightBracket,
          .leftParen, .text, .rightParen, .eof,
        ],
        "Links and images"
      ),
      (
        ...
        [
          .text, .space, .text, .space, .dash, .space, .asterisk, .asterisk, .text, .asterisk,
          .asterisk, .space, .text, .space, .text, .space, .dash, .space, .asterisk, .text,
          .asterisk, .space, .text, .eof,
        ],
      ...
      (
        ...
        [
          .text, .space, .text, .space, .gt, .space, .asterisk, .asterisk, .text, .asterisk,
          .asterisk, .space, .text, .space, .text, .space, .gt, .space, .asterisk, .text, .asterisk,
          .space, .text, .eof,
        ],
      ...
      (
        ...
        [
          .text, .space, .text, .space, .formula, .space, .text, .space, .text, .space, .text,
          .space, .inlineCode, .eof,
        ],
        "Math and code"
      ),
      (
        ...
        [
          .text, .space, .text, .space, .htmlBlock, .space, .text, .space, .text, .space, .asterisk,
          .asterisk, .text, .asterisk, .asterisk, .eof,
        ],
        "HTML and markdown"
      ),
    ]
    ...
    for (input, expectedElements, description) in testCases {
      ...
      #expect(actual == expectedElements, "Token sequence mismatch for: \(description)")
    }
  }
  ...
  func testMixedComplexSyntax() {
    let testCases: [(String, [MarkdownTokenElement], String)] = [
      (
        ...
        [
          .asterisk, .asterisk, .text, .space, .text, .space, .inlineCode, .space, .text, .asterisk,
          .asterisk, .eof,
        ],
        "Bold with code"
      ),
      (
        ...
        [
          .asterisk, .text, .space, .text, .space, .asterisk, .asterisk, .text, .asterisk,
          .asterisk, .space, .text, .asterisk, .eof,
        ],
        "Italic with bold"
      ),
      (
      ...
      (
        ...
        [
          .htmlEntity, .space, .text, .space, .text, .space, .asterisk, .asterisk, .text, .asterisk,
          .asterisk, .space, .text, .eof,
        ],
        "Entity with bold"
      ),
      (
        ...
        [
          .hash, .space, .text, .space, .text, .space, .formula, .space, .text, .space,
  ...
  func testComplexDocumentStructure() {
    ...
    #expect(tokens.count > 100, "Should produce many tokens for complex document")
    ...
    // Basic tokens
    #expect(elements.contains(.hash), "Should contain hash tokens")
    #expect(elements.contains(.asterisk), "Should contain asterisk tokens")
    #expect(elements.contains(.dash), "Should contain dash tokens")
    #expect(elements.contains(.pipe), "Should contain pipe tokens")
    #expect(elements.contains(.gt), "Should contain gt tokens")
    #expect(elements.contains(.leftBracket), "Should contain left bracket tokens")
    #expect(elements.contains(.rightBracket), "Should contain right bracket tokens")
    #expect(elements.contains(.leftParen), "Should contain left paren tokens")
    #expect(elements.contains(.rightParen), "Should contain right paren tokens")

    // Code tokens (now as complete tokens)
    #expect(elements.contains(.inlineCode), "Should contain inline code tokens")

    // HTML tokens
    #expect(elements.contains(.htmlBlock), "Should contain HTML block tokens")
    #expect(elements.contains(.htmlComment), "Should contain HTML comment tokens")
    #expect(elements.contains(.htmlEntity), "Should contain HTML entity tokens")

    // Math tokens
    #expect(elements.contains(.formula), "Should contain formula tokens")
    #expect(elements.contains(.formulaBlock), "Should contain formula block tokens")
  }

  func testPerformanceWithLargeDocument() {
    // Create a reasonably large document
    let largeText = Array(
      repeating: "This is a paragraph with **bold** and *italic* text. ", count: 50
    ).joined()

    let (tokens, _) = tokenizer.tokenize(largeText)

    #expect(tokens.count > 50, "Should produce many tokens for large document")
    #expect(tokens.last?.element == .eof, "Should end with EOF")
  }
  ...
  func testComplexCodeBlockScenarios() {
    let testCases: [(String, String, [MarkdownTokenElement])] = [
      ...
      // Mixed content
      (
        "Some text\n```swift\ncode\n```\nMore text", "Text with fenced code block",
        [.text, .space, .text, .newline, .fencedCodeBlock, .newline, .text, .space, .text, .eof]
      ),
      ...
      // Inline code
      (
        "This is `inline code` in text", "Inline code in text",
        [.text, .space, .text, .space, .inlineCode, .space, .text, .space, .text, .eof]
      ),
      (
        "Multiple `code1` and `code2` inline", "Multiple inline code blocks",
        [.text, .space, .inlineCode, .space, .text, .space, .inlineCode, .space, .text, .eof]
      ),
    ]
    ...
    for (input, description, expectedElements) in testCases {
      ...
      #expect(
        actualElements.count == expectedElements.count,
      ...
      for (index, expectedElement) in expectedElements.enumerated() {
        if index < actualElements.count {
          #expect(
            actualElements[index] == expectedElement,
  ...
  func testUnclosedCodeBlockEdgeCases() {
    ...
    for (input, description) in testCases {
      ...
      if input.starts(with: "```") {
        ...
        #expect(
          tokens.first?.element == .fencedCodeBlock,
          "Should be fenced code block for: \(description)")
        #expect(
          tokens.first?.text == input,
